...
homology_list <- read_xlsx("data/DOK084-072 Summary of results in PD084.xlsx", sheet = "Homology and Wish") %>%
  select(1:3) %>%
  clean_names() %>%
  distinct()
...
assay_status_result <- get_assay_status(
  uniprot_list,
  assay_categories,
  antigen_categories,
  in_product,
...
assay_status_result %>%
  group_by(assay_status_antigen_status) %>%
  count() %>%
  arrange(desc(n)) %>%
  View()
...
assay_status_result_compare <-
  assay_status_result %>%
  left_join(angelica %>% 
              select(uniprot, "Status AEH")) %>%
  relocate(`Status AEH`, .after = uniprot)

write_xlsx(assay_status_result_compare, "assay_status_resut_compare_2025_11_17.xlsx")
...
uniprot_list %>% 
  filter(!uniprot %in% assay_status_result$uniprot)
